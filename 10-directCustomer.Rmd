...
Total purchased electricity from EGAT direct customers decreased from 3,831 GWh in 2010 and reached its minimum 2,392 GWh in 2023. The purchased electricity gradually increases to 2,744 GWh in 2037 (see Figure \@ref(fig:plot-directCustomer)). Figure \@ref(fig:plot-dcdomestic) and \@ref(fig:plot-dcneighboring) shows a purchased electricity from a domestic and a neighboring customers. **Sattahip navy station** purchased the highest electricity, followed by **SCG Tungsong** and **SCG Talung** (see Figure \@ref(fig:plot-dcdomestic)). EDL and EDC are the top purchased elecricity from EGAT (see Figure \@ref(fig:plot-dcneighboring)). 
...
```
```{r plot-dcdomestic, echo=FALSE, warning=FALSE, message=FALSE, fig.cap='Purchased electricity by domestic customers', fig.align='center', out.width= '90%'}
full_join(
  #1
  read_excel("raw_data/01 EGAT_20Oct2022_งบ66 67_Final_Sent อศง.xlsx",
           sheet = "DCs",
...
pivot_wider( names_from = name, values_from = ene_gwh) %>% 
mutate(total_domestic = rowSums(across(where(is.numeric)), na.rm=TRUE)) %>% 
pivot_longer(-year&-region, names_to = "name", values_to = "ene_gwh")  %>% 
ggplot(aes(x = as.numeric(year), y = ene_gwh))+
  ...
  facet_wrap(~name,
             ...
             labeller = as_labeller(c(sattahip = "Sattahip navy station", 
                                      ...
                                      temporary = "Temporary",
                                      thaioil = "Thai Oil",
                                      total_domestic = "TOTAL")
                                    )
             )+
  scale_color_manual(values = linepalette2)+
  ThemeLine+
...
```{r plot-dcneighboring, echo=FALSE, warning=FALSE, message=FALSE, fig.cap='Purchased electricity by neighboring customers', fig.align='center', out.width= '90%'}
full_join(
  #1
  read_excel("raw_data/01 EGAT_20Oct2022_งบ66 67_Final_Sent อศง.xlsx",
           sheet = "DCs",
...
pivot_longer(-c(direct_cus,name,region), names_to = "year", values_to = "ene_gwh") %>%
select(-direct_cus) %>%
filter(region == "neighboring") %>%
pivot_wider( names_from = name, values_from = ene_gwh) %>% 
mutate(total_neighboring = rowSums(across(where(is.numeric)), na.rm=TRUE)) %>% 
pivot_longer(-year&-region, names_to = "name", values_to = "ene_gwh")  %>% 
ggplot(aes(x = as.numeric(year), y = ene_gwh))+
  geom_line(aes(group = name, color = name),
            show.legend = FALSE)+
  facet_wrap(~name,
             scales = "free_y",
             labeller = as_labeller(c(edc = "EDC",
                                      tnb = "TNB",
                                      edl = "EDL",
                                      TheunHinboun = "Theun Hin boun hydro",
                                      HouayHo = "Houay Ho hydro",
                                      NamTheun2 = "Nam Theun 2 hydro", 
                                      NamNgum2 = "Nam Ngum 2 hydro",
                                      Hongsa = "Hongsa Thermal",
                                      XePianXeNamnoy = "Xe-Pian Xe-Namnoy",
                                      NamNgiep1 = "Nam Ngiep 1 hydro",
                                      Xayaburi = "Xayaburi hydro",
                                      NamTheun1 = "Nam Theun 1 hydro",
                                      total_neighboring = "TOTAL")
                                    )
             )+
  scale_color_manual(values = linepalette2)+
  ThemeLine+
